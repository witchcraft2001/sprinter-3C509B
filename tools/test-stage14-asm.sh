...
# 4. Run the cold-overlay vectors against the SHIPPED blob at its real ORG.
#    Nothing else in the automated suite executes a cold instruction, and the
#    cold routines are rewrites (fixed hot scratch became register scratch),
#    so their register contracts need their own coverage.
(
  cd "$tmp_dir"
  sjasmplus --nologo --fullpath -I "$repo_root/src/include" -I "$tmp_dir" \
    -DVEC_BASE=0x2000 --sym="$tmp_dir/vec_cold.sym" \
    "$script_dir/stage14_cold_vectors.asm" >"$tmp_dir/vec_cold.log" 2>&1
)
if grep -Eq 'Errors: [1-9]|error:' "$tmp_dir/vec_cold.log"; then
  cat "$tmp_dir/vec_cold.log" >&2
  exit 1
fi
cold_end="$(awk '/^TEST_DONE:/ {sub(/^0x0*/, "", $3); print $3}' "$tmp_dir/vec_cold.sym")"
z88dk-ticks -l 0 -pc 0x2000 -end "$cold_end" -counter 100000000 \
  -output "$tmp_dir/vec_cold.ram" "$tmp_dir/cold_vectors.bin" >/dev/null
cold_result="$(od -An -tu1 -j 16128 -N 1 "$tmp_dir/vec_cold.ram" | tr -d ' ')"
cold_complete="$(od -An -tu1 -j 16129 -N 1 "$tmp_dir/vec_cold.ram" | tr -d ' ')"
if [ "$cold_complete" != 165 ] || [ "$cold_result" != 0 ]; then
  echo "Error: Stage 14 cold-overlay vector failed at case $cold_result (complete=$cold_complete)" >&2
  exit 1
fi
# Deepest stack the vectors saw a cold call use, measured on a poisoned stack
# (case 150-153). COLD.RUN gives the blob a 96-byte private stack inside the
# DLL image, with the dispatcher's own scalars and COLD_CTX directly beneath
# it, so this number is worth watching as cold routines grow.
cold_depth="$(od -An -tu2 -j 16132 -N 2 "$tmp_dir/vec_cold.ram" | tr -d ' ')"
