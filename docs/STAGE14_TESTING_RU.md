...
## Сценарий G: FIN пира во время SEND

Этот сценарий воспроизводит ранний HTTP-ответ: сервер не дочитывает тело,
отправляет статус и FIN, пока текущий блок клиента подтверждён лишь частично.
На macOS сначала отключите автонастройку receive buffer:

```sh
sudo sysctl -w net.inet.tcp.doautorcvbuf=0
python3 tools/host/unettest_fin_during_send.py --bind 192.168.7.44 --port 8080
sudo sysctl -w net.inet.tcp.doautorcvbuf=1
```
...
Лог скрипта должен показать `accepted socket rcvbuf=700` (допустимо близкое
значение ядра) и от 1 до 1199 непрочитанных байтов в момент FIN, без
`WARNING`. Ожидаемая диагностика DLL: `st=03 nerr=07 tcp=20`. Это
`NERR_CLOSED` и сырой `TCP_ERR_CLOSED`; `nerr=05` означал бы старый общий
send-failure. Сохраните консоль, лог сервера и pcap. Автоматические векторы
дополнительно доказывают, что ответ переживает ошибку `SEND`, читается через
успешный `RECV`, а следующий `RECV` освобождает канал с `NERR_CLOSED`.

## Сценарий H: что CLOSE сообщает пиру

С 0.1.3 `CLOSE` ведёт себя как в `UNETRTL.DLL` 0.3.10. Если всё отправленное
подтверждено, уходит FIN|ACK. Он повторяется до трёх раз с тем же seq, с
ожиданием по 500 мс. Если после SEND остался неподтверждённый сегмент,
вместо FIN уходят два RST|ACK: первый на `SND.NXT`, второй на `SND.UNA`.
`UNETTEST` код возврата CLOSE не печатает, у сиблинга так же. Поэтому
доказательством служит pcap, а не экран. Захват ставьте до запуска:

```sh
sudo tcpdump -ni feth1 -w close.pcap "tcp port 8080"
```

**H1. Штатное закрытие со стороны Sprinter.** Responder держит соединение
дольше, чем `UNETTEST` читает ответ (4 блока по 4 с):

```sh
tools/stage14-mame.sh responder tcp-echo --port 8080 --timeout 30
```
...
Ожидается `--- reply ---` с эхом и `RESULT OK`. Строки `--- closed ---` нет:
первым закрывает Sprinter. В pcap примерно через 16 с после запроса должен
быть ровно один `Flags [F.]` от `192.168.7.21`, сразу за ним ACK хоста на
`seq+1`, и ни одного `Flags [R.]` от Sprinter. В логе responder'а
`echoed N bytes, closing` появляется сразу после этого FIN, а не через 30 с.

**H2. Отмена SEND (Esc) и немедленный повтор.** Responder не читает сокет
20 с, поэтому сегмент `UNETTEST -a` висит неподтверждённым:

```sh
sudo sysctl -w net.inet.tcp.doautorcvbuf=0
tools/stage14-mame.sh responder tcp-stall --port 8080 --rcvbuf 700 --stall 20
```
...
Предупреждение responder'а о бюджете resume здесь ожидаемо. Когда на экране
пойдут строки `SEND suspended (NERR_AGAIN), confirmed so far: ...`, нажмите
Esc. На это есть около 3 с: 20 возобновлений по 150 мс. `UNETTEST` взводит
CANCELKEYS всегда. Ожидается `Send failed.`, `lasterr: ... st=03 nerr=08` и
`RESULT FAIL code=3`.

Если Esc не успел, будет `gave up after too many NERR_AGAIN resumes`. Тогда
CLOSE внутри `UNETTEST` получает `NERR_BUSY`, а соединение закрывает выгрузка
DLL (FINI). В проводе результат тот же, но это не тот путь, повторите
попытку.

- **Если в pcap после последнего сегмента данных Sprinter есть
  неподтверждённые байты:** сразу за Esc идут два `Flags [R.]` от Sprinter.
  Их seq отличаются ровно на длину этого сегмента: первый — конец
  сегмента, второй — его начало. `Flags [F.]` от Sprinter нет.
- **Если окно было закрыто и данных в полёте не было:** уходит один
  `Flags [F.]`. Это тоже правильно.

Сразу, не дожидаясь responder'а, повторите ту же команду. Второй запуск
должен установить новое соединение (новый исходный порт в pcap). Первое
соединение в логе responder'а должно завершиться сбросом, а не висеть до
конца `--stall`. После теста верните
`sudo sysctl -w net.inet.tcp.doautorcvbuf=1`.

**H3. Не хватает только железа.** Тишину пира после FIN (`NERR_TIMEOUT`
через 1,5 с), FIN, не ушедший в провод (`NERR_HW`), Esc в ожидании FIN
(`NERR_CANCEL`), данные второго канала во время закрытия и выбор статуса
`NETDONE` исполняют автоматические векторы. В MAME их не повторяем.
...
### 8. Закрытие и повтор после отмены

Это проверка из багрепорта SNC. Повтор прерванной передачи должен проходить
сразу, как с `UNETRTL.DLL`. Захват на хосте обязателен: результат CLOSE
виден только в проводе.

```sh
sudo tcpdump -ni any -w close-hw.pcap "host SPRINTER and tcp"
```

1. **Штатное закрытие.** Выполните сценарий H1 с `--bind HOST`. В pcap
   один `Flags [F.]` от Sprinter, ACK хоста, ни одного RST от Sprinter.
2. **Отмена и повтор.** Выполните сценарий H2 с `--bind HOST`. Правило для
   pcap то же: два `Flags [R.]` на двух seq, если данные были в полёте, и
   один `Flags [F.]`, если нет. Повторный запуск проходит сразу.
3. **SNC.** Запустите загрузку файла на FTP- или WebDAV-сервер в сети,
   прервите её и сразу повторите ту же операцию.
   - **Ожидается:** второй запрос завершается без ожидания idle-таймаута
     сервера.
   - **Для WebDAV:** нет `HTTP 423 Locked`.
   - **В pcap:** у первого соединения RST или FIN от Sprinter сразу после
     отмены.

Сохраните:

- экран (или фото) каждого шага;
- `close-hw.pcap`;
- лог сервера с обоими TCP-кортежами;
- sha256 `UNET509B.DLL`;
- что именно использовалось в шаге 3: FTP или WebDAV, сервер и его версия.

## Доказательства

Соберите для `docs/evidence/STAGE14_TEST_TEMPLATE.md`: скриншоты/транскрипты
сценариев 0 и A-H; логи `stage14_responder.py` для каждого сценария; sha256
