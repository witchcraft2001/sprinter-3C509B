; DLDIRECT.EXE - direct HTTP/1.0 download throughput measurement over the
; 3C509B backend. Development/measurement tool only: IMG, never the ZIP.
; SPDX-License-Identifier: BSD-3-Clause
;
; The body is discarded as it arrives -- no DLL and no disk write -- so
...
; single-channel receive window (both keyed off STAGE12_LAYOUT). DLDIRECT
...
HTTP_STATE_VAR		EQU W12_HTTP_STATE
HTTP_STATUS_VAR		EQU W12_STATUS
HTTP_HEADER_LENGTH_VAR	EQU W12_HEADER_LENGTH
HTTP_CONTENT_KNOWN_VAR	EQU W12_CONTENT_KNOWN
HTTP_CONTENT_LENGTH_VAR	EQU W12_CONTENT_LENGTH
HTTP_BODY_RECEIVED_VAR	EQU W12_BODY_RECEIVED
HTTP_WORK32_VAR		EQU W12_WORK32
...
.ARGS_OK
	...
	; Construct the request outside the measurement. After CONNECT and the RTC
	; edge, TIME_START is therefore immediately followed by SEND.
	CALL	BUILD_GET
	CALL	@TCPX.RESET
	XOR	A
	LD	HL,NET_TARGET_IP
...
; here rather than once each. STAGE9_FILE_BUFFER is otherwise unused -- DLDIRECT
; discards the body, and the shared HTTP parser consumes whatever buffer it is
; handed.
.RX_LOOP
	XOR	A
	...
	; http.server, and DLDIRECT's "Connection: close" is only a request) is
